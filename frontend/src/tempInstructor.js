...
function Instructor() {
  ...
  for (i = 0; i < Math.floor(names.length / 3); i++) {
      ...
      cols.push(
        <Col xs={12} sm={4} className="person-wrapper" key={j}>
          <InstrucCard
            imgSrc={imgs[count]}
            name={names[count]}
            shortDesc={shortDescs[count]}
            longDesc={longDescs[count]}
          />
        </Col>
      );
      count++;
    ...
    rows.push(
      <Row className="show-grid text-center" key={i}>
        {cols}
      </Row>
    );
  }
